...
String result = responseData['result']; // Get the value associated with the 'result' key
int idUtente = int.parse(responseData['idUtente']); // Parse the value associated with the 'idUtente' key as an integer

// Extract the list of products from the response data
List<Map<String, dynamic>> productList = List<Map<String, dynamic>>.from(responseData['listOfProduct']);
List<Product> listProd = productList.map((item) => Product.fromJson(item)).toList();

// Extract the list of product IDs from the wishlistDTO
List<int> wishlistDTO = List<Map<String, dynamic>>.from(responseData['wishListDTO'])
